...
class EntryListItem extends StatelessWidget {
  const EntryListItem(
      {@required this.onTap, @required this.entry, @required this.job});
  final Entry entry;
  final Job job;
...
class DismissibleEntryListItem extends StatelessWidget {
  const DismissibleEntryListItem(
      {this.entry, this.onTap, this.job, this.key, this.onDismissed});
  final Key key;
  final Entry entry;
  final Job job;
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      ...
      child: EntryListItem(onTap: onTap, entry: entry, job: job),
    );
  }
}
